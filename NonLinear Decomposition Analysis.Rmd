...
- Note that the hessian returned by optim may be numerically unstable, try to implement with numDeriv::hessian() - (though this requires modification to the objective function in litterfitter to have the parameters directly specified in the obj func wrapper)
- See 'test_new_litterfitter_functionality.R" for test workspace and load of litterfitter fork and 'internal.R' from litterfitter fork.
 ...
 solve() does this inversion - 
"In the unlikely event that you are maximizing the likelihood itself, you need to divide the negative of the hessian by the likelihood to get the observed information [matrix]." `solve(-fit$hessian/fit$value)` -
