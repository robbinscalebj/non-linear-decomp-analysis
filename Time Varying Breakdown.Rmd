...
coweeta_pairs <- coweeta_df|>ungroup()|>
  filter(species != "WOOD")|>
  select(pair_id, mesh.type, days.incubated, mass_prop)|>
  group_by(pair_id)|>nest()|>
  slice_head(n = 1)
...
# Loop
test <- coweeta_pairs|>pluck("data", 1)

pairs_fit <- brm(weibullpairs_bf, 
                 data = test, 
                 prior = weibullcohort_priors)

test_grid <- expand_grid(days.incubated = seq(0,125, by = 1), mesh.type = c("Coarse", "Fine"))
...
test_draws <- add_epred_draws(pairs_fit, newdata = test_grid, ndraws = 999)

summarized_draws <- test_draws|>group_by(mesh.type, days.incubated)|>
